...
# precheck
UTIL_LIST="iostat mpstat pidstat sar ethtool"
which $UTIL_LIST || err_exit "please add to execute path for all utils: [$UTIL_LIST]" 
...
# info pre
echo `basename $0`-$VER > $LOG_INFO
uptime >> $LOG_INFO
uname -a >> $LOG_INFO
which flash_util && flash_util -l
lsmod >> $LOG_INFO
sysctl -a | grep mem >> $LOG_INFO
cat /proc/cpuinfo >> $LOG_INFO
...
sar -n DEV,EDEV,SOCK,IP,EIP,TCP,UDP $INTERVAL $COUNT > $LOG_NETWORK &
